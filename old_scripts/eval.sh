...
for (( i = 0; i < $rounds; i++ )); do
    ...
    if [[ "$ab" == "on" ]]; then
    	...
    	sudo cp /proj/ntucsie-PG0/fjyang/cloud-hack-ab-stress-bak.img /proj/ntucsie-PG0/fjyang/cloud-hack-ab-stress.img
    else
    ...
    if [[ $ab == "on" ]]; then 
        ...
        /users/fjyang/httpd-2.4.54/support/ab -c 100 -n 1000000 -s 30 -g "$ab_fn" http://10.10.1.5/ >&2 & 
        ab_pid=$!
	sleep 3s
    fi
