...
migration_port=8888
rounds=10
output_dir_root="./eval-data-ab-1G"
result_prefix="res-ab-1G"
...
MultiFD_channels[1]="1"
MultiFD_channels[2]="2"
MultiFD_channels[3]="4"
...
mutual_migration_attr[1]="migrate_set_parameter downtime-limit 5000"
...
EOF
        ...
        cd /mydata/some-tutorials/files/blk
        sudo nohup ./resume-blk.sh --bridge
EOF
        } 2>&1 )
...
EOF
        ...
        err=$(echo "$log" | grep "qemu-system-aarch64: -monitor telnet:0.0.0.0:1234,server,nowait: Failed to find an available port: Address already in use")
        err+=$(echo "$log" | grep "qemu-system-aarch64: -monitor telnet:0.0.0.0:1235,server,nowait: Failed to find an available port: Address already in use")
        if [[ -n "$err" ]]; then
            echo -e "${BRED}socket broken, rebooting${NC}"      
            ssh $username@$src_ip << EOF
            sudo reboot
EOF
            ssh $username@$dst_ip << EOF
...
EOF
        ...
        for attr in "${src_migration_attr[@]}"; do
            ncat -w 5 -i 2 $src_ip $src_monitor_port <<< "$attr"
        done
	...
	echo ""
        ...
        for attr in "${dst_migration_attr[@]}"; do
            ncat -w 5 -i 2 $dst_ip $dst_monitor_port <<< "$attr"
        done
        for attr in "${mutual_migration_attr[@]}"; do
            ncat -w 5 -i 2 $dst_ip $dst_monitor_port <<< "$attr"
        done
	...
	ab -c 100 -n 100000000000000000 -s 60 -g "$output_dir/ab-$t-$i" http://10.10.1.5/ &
	#ab -c 10 -n 100000000000000 -s 60 -g log http://10.10.1.5/ > /dev/null < /dev/null &
	pid=$!
	echo "$pid"
	...
	echo ""
        ...
        ncat -w 10s -i 2 $src_ip $src_monitor_port <<< "$command_info" | strings > "$output_dir/src_$i.txt"
        ncat -w 10s -i 2 $dst_ip $dst_monitor_port <<< "$command_info" | strings > "$output_dir/dst_$i.txt"
	echo ""
	...
	if [ ! -e "$output_dir/ab-$t-$i" ]; then
	#if [ ! -e "ab-$t-$i" ]; then
            echo -e "${BRED}ab failed${NC}"
	    failed="True"
	fi
        ...
        if [[ "$src_downtime_limit" != "5000" ]]; then
        ...
        if [[ "$dst_downtime_limit" != "5000" ]]; then
